...
実行後、指定した `src/*.md` に対応する HTML が `HTML/` に出力されます。対象ファイルを省略した一括生成は行いません。生成 HTML には、通常の画面表示、高齢者向け印刷、紙節約印刷、スライド表示、印刷/PDF 出力の各モードが内包されています。図版は自動生成・自動挿入しません。
...
Markdown 正本から説明用スライド（`.pptx`）を生成するには、以下を実行します。スライド要約または本文抜粋がスライドに反映され、スピーカーノートに Markdown の詳細テキストが自動流し込みされます。図版は自動生成・自動挿入しません。
